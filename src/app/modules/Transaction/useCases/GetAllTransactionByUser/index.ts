...
export default async function GetAllTransactionByUser(userId: string): Promise<{
  itemsPerPage: number,
  page: number,
  ...
  const transactions = await TransactionRepository.findAllTransactionsByUser(findUser.id);
